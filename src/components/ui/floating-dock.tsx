            ...
            {items.map((item, idx) => (
              ...
              >
                <Link
                  href={item.href}
                  key={item.title}
                  className="h-10 w-10 rounded-full bg-foreground flex items-center justify-center"
                >
...
const FloatingDockDesktop = ({
  ...
  return (
    <motion.div
      ...
      className={cn(
        "mx-auto hidden md:flex h-16 gap-4 items-end rounded-2xl bg-muted/10 px-4 pb-3",
        className
      )}
    >
      {items.map((item) => (
